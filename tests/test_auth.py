...
class TestGoogleDriveClient:
    ...
    def test_authenticate_existing_valid_token(self, mock_exists, mock_from_file, mock_credentials):
    ...
    @patch('os.path.exists')
    def test_authenticate_expired_token_refresh(self, mock_exists, mock_request, mock_from_file):
    ...
    def test_authenticate_new_flow(self, mock_exists, mock_flow):
        ...
        assert result is True
        assert client._creds == mock_creds
        mock_flow.assert_called_once_with("credentials.json", [
            'https://www.googleapis.com/auth/drive',
            'https://www.googleapis.com/auth/documents'
        ])
    ...
    def test_drive_service_property(self, mock_build, mock_google_client):
        ...
        mock_build.assert_called_once_with('drive', 'v3', credentials=mock_google_client._creds)
    ...
    def test_docs_service_property(self, mock_build, mock_google_client):
        ...
        assert service == mock_service
        mock_build.assert_called_once_with('docs', 'v1', credentials=mock_google_client._creds)